...
const IncomingWasteLogEntry = () => {
  ...
  useEffect(() => {
    axios
      .get(
        `http://localhost:8000/thirdparties/getcompanyforsts/${JSON.parse(
  ...
  const handleSubmit = (event) => {
    ...
    if (!data.company || !data.timeOfArrival || !data.type || !data.vehicle || !data.amount) {
      Swal.fire({
        icon: "error",
        title: "Oops...",
    ...
    axios
      .post(`http://localhost:8000/thirdparties/incomingstsentry`, {
      ...
      >
        <div className="mb-4">
          ...
          <select name="company" className="select select-bordered w-full">
            {companies.map((c) => (
              <option key={c.name_of_the_company} value={c.contract_id}>
