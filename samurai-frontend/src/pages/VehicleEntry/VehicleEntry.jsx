...
const VehicleEntry = () => {
    ...
    useEffect(() => {
        axios
            .get(
                "https://cs24-p2-ontorponthik.onrender.com/vehicle/allstsvehicle"
            )
            .then((res) => {
                const registrationNumbers = res.data?.vehicles?.map(
                    (vehicle) => vehicle.registration_number
                );
                setVehicles(registrationNumbers);

                const vehiclesRegistrationNumbersAndTypes =
    ...
    useEffect(() => {
        axios
            .get(
                `https://cs24-p2-ontorponthik.onrender.com/vehicle/alllandfillvehicle/${JSON.parse(
    ...
    const handleSTSChange = (e) => {
        ...
        if (value.length > 0) {
            ...
            if (name === "vehicleRegNo") {
                ...
                const matchedVehicle = vehiclesType.find(
                    (vehicle) => vehicle.reg_no === value
                );
                ...
                if (matchedVehicle) {
                    ...
                    ).innerHTML = `Maximum Capacity: ${matchedVehicle.capacity} Ton`;
                } else {
                    const txtBx = document.getElementById("stsVehicleType");
                    txtBx.value = "";
                    document.getElementById(
    ...
    const onSuggestionClickLandfill = (value) => {
        const matchedVehicle = availableLandfillVehicles.find(
            (vehicle) => vehicle.registration_number === value
        );
        ...
        if (matchedVehicle) {
            ...
            document.getElementById("landfillLoadCarrying").value =
                matchedVehicle.carring_weight;
            document.getElementById("fromStsWardNumber").value =
                matchedVehicle.from_sts;
        } else {
            document.getElementById("landfillLoadCarrying").value = "";
            document.getElementById("fromStsWardNumber").value = "";
        }
        ...
        return (
            <ul className="suggestions border-2 rounded-lg p-3 shadow-md">
                {suggestions.map((suggestion, index) => (
                    <li
                        className="cursor-pointer hover:bg-gray-200 py-2 px-4 rounded-lg"
                        key={index}
            ...
            <ul className="suggestions border-2 rounded-lg p-3 shadow-md">
                {suggestions.map((suggestion, index) => (
                    <li
                        className="cursor-pointer hover:bg-gray-200 py-2 px-4 rounded-lg"
                        key={index}
                        onClick={() => onSuggestionClickLandfill(suggestion)}
                    >
                        {suggestion}
                    </li>
                ))}
    ...
    const handleSubmit = (e) => {
        ...
        const selectedVehicle = vehiclesType.find(
            (vehicle) => vehicle.reg_no === stsFormData.vehicleRegNo
        );
        if (
            selectedVehicle &&
            parseFloat(stsFormData.weightCarrying) >
                parseFloat(selectedVehicle.capacity)
        ) {
            Swal.fire({
                icon: "error",
                title: "Oops...",
        ...
        axios
            .post(
                "https://cs24-p2-ontorponthik.onrender.com/sts/entry",
                postDataInfo
            )
            .then((res) => {
                setSpinner(false);
    ...
    const handleChange = (e) => {
        ...
        // Give vehicle suggestions to the user
        if (value.length > 0) {
            const filteredSuggestions = availableLandfillVehicles
                ? availableLandfillVehicles.filter((vehicle) =>
            ...
            const filteredSuggestionNames = filteredSuggestions.map(
    ...
    const handleLandfillEntry = (e) => {
        ...
        setSpinner(true);
        axios
            .post("https://cs24-p2-ontorponthik.onrender.com/landfill/lentry", {
                token: JSON.parse(localStorage.getItem("user")),
            ...
            .then((res) => {
                setSpinner(false);
                if (
                    res.data?.message === "LandfillEntry created successfully"
                ) {
                    Swal.fire({
                        title: "Good job!",
                        text: "Entry Added!",
            ...
            >
                ...
                <div className="mb-2">
                    <label
                        ...
                        className="block mb-1 text-green-500 text-right"
                    >
