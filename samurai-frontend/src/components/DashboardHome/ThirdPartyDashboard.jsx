...
const ThirdPartyDashboard = () => {
    ...
    useEffect(() => {
        axios
            .get(
                `https://cs24-p2-ontorponthik.onrender.com/thirdparties/getcompanyname/${JSON.parse(
    ...
    useEffect(() => {
        axios
            .get(
                `https://cs24-p2-ontorponthik.onrender.com/thirdparties/outgoinglogs/${JSON.parse(
                    localStorage.getItem("user")
                )}`
            )
            .then((res) => {
    ...
    useEffect(() => {
        ...
        axios
            .get(
                `https://cs24-p2-ontorponthik.onrender.com/thirdparties/getemployeelogs/${JSON.parse(
                    localStorage.getItem("user")
                )}?is_today=false`
            )
            .then((res) => {
    ...
    useEffect(() => {
        axios
            .get(
                `https://cs24-p2-ontorponthik.onrender.com/thirdparties/getemployeelogsforlast/${JSON.parse(
                    localStorage.getItem("user")
                )}`
            )
            .then((res) => {
                ...
                <div className="overflow-x-auto">
                    ...
                    <div className="flex justify-between items-center my-3">
                        <div className="flex flex-col gap-1 text-green-500 font-semibold">
                            <span>
                                Total Payment: {logs?.summary?.totalPayment} BDT
                            </span>
                            <span>
                                Total Waste:{" "}
                                {parseFloat(
                                    logs?.summary?.totalWasteCarriedTons
                                ) * 1000}{" "}
                                kg
                            </span>
                        </div>
                            ...
                            {outgoingLogs &&
                                outgoingLogs.map((log, idx) => (
                                    <tr key={log._id}>
                                        ...
                                        <td>
                                            {
                                                log.vehicle_used_for_transportation
                                            }
                                        </td>
                                        <td>{log.fine}</td>
