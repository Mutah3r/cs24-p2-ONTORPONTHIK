...
exports.getSTSInformation = async (req, res) => {
  try {
    ...
    const stsWithManager = await Promise.all(
      stsInfo.map(async (sts) => {
...
exports.getLandfillInformation = async (req, res) => {
  try {
    ...
    const landfillsWithManager = await Promise.all(
      landfillInfo.map(async (landfill) => {
        // Find the manager by their MongoDB ID
...
>>>>>>> 350be7eb5199516748755f4256ff708e8f80912f
    ...
    res
      .status(200)
      .json({ message: "Manager assigned to Landfill successfully" });
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: "Internal server error" });
  }
